...
if (! empty($_GET['id'])) {
	...
	// Begin RSS File
	$feed .= "<title>" . $category['name'] . " | " . NAME . " | " . $db->get_option('company_name') . "</title>\n";
	$feed .= "<link>$link</link>\n";
	...
	// Get articles
	$q = "SELECT id,category,name,created,meta_desc FROM `" . TABLE_PREFIX . "articles` WHERE `category`='" . $db->mysql_clean($_GET['id']) . "' AND `public`='1' ORDER BY `created` DESC $limit";
