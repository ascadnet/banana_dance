...
if ($privileges['is_admin'] != "1") {
	...
	$themes = $admin->get_theme_list('array','1');
...
<div id="content_overlay">
	...
	<div class="submit">
   		...
   		<a href="http://www.doyoubananadance.com/Download/Mobile-Theme-Library" target="_blank"><img src="imgs/icon-download.png" width="16" height="16" border="0" title="Download New" alt="Download New" /></a>
	</div>
	
	<div id="actions_right">
		<ul>
			<li><a href="index.php?l=themes">Switch to Standard Themes</a></li>
		</ul>
	</div>
	...
	<h1>Mobile Themes</h1>
		...
		foreach ($themes as $aTheme) {
			// Status
			$show_status = "<a href=\"#\" onclick=\"setTheme('" . $aTheme['folder_name'] . "','2');\">";
			...
			$show_status .= "</a>";
			// Screen?
			$screen = PATH . "/templates/mobile/" . $aTheme['folder_name'] . "/_author/screen.jpg";
	   		if (file_exists($screen)) {
	   			$link = URL . "/templates/mobile/" . $aTheme['folder_name'] . "/_author/screen.jpg";
				$screen = "<a href=\"$link\" target=\"_blank\" /><img src=\"$link\" width=\"90\" height=\"177\" border=\"0\" alt=\"Screen shot\" title=\"Screen shot\" /></a>";
			} else {
				$screen = "N/A";
			}
			// Developer
			...
			echo "<td valign=\"top\">
				...
				<p class=\"small\">By $final_dev</p>";
				if (! empty($aTheme['description'])) {
					echo "<p style=\"margin-top: 12px;\" class=\"desc\">" . $aTheme['description'] . "</p>";
				}
			echo "<p style=\"margin: 12px 0 12px 0;\" class=\"small\"><a href=\"functions/import_template_imgs.php?type=mobile&theme=" . $aTheme['folder_name'] . "\">Import Images to Media Gallery</a></p>";
			echo "</td>\n";
			echo "</tr>\n";
		}
